...
export function Loader(props : LoaderProps) {
    ...
    const getSpinnerSvgString = () : string => {
        ...
        <circle class="opacity-25" cx="12" cy="12" r="10" stroke="${
            properties ?. spinner ?. colorStroke || '#e4e4e4'
        }" stroke-width="4"></circle>
        <path class="opacity-75" fill="${
            properties ?. spinner ?. colorFill || '#0f172b'
        ...
        return (
            <view className="flex flex-col items-center justify-center gap-1 ">
                <view className='w-full max-w-sm '>
                    <view style={loaderBackgroundColorStyle}
                        className="h-2 overflow-hidden rounded-full">
                        <view style={loaderPercentageStyle}
                            className={
                                properties ?. loader ?. animate ? "h-full animate-pulse" : "h-full"
                        }></view>
                    </view>
                </view>
                {
                label ? <text style={loaderTextColorStyle}
